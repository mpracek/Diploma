...
\begin{algorithm}
	...
	;OUTPUT
	*The most likely hidden state sequence <math> X=(x_1,x_2,\ldots,x_N) </math>
	'''function''' ''VITERBI''<math>(O,S,\Pi,Y,A,B):X</math>
	'''for''' each state <math>j \in \{1,2,\ldots,K\}</math> '''do'''
	{{math|''T''<sub>1</sub>[''j'',1] ? ''?<sub>j</sub>''&middot;''B''<sub>''jy''<sub>1</sub></sub>}}
	{{math|''T''<sub>2</sub>[''j'',1] ? 0}}
	'''end for'''
	'''for''' each observation <math>i = 2,3,\ldots,T</math> '''do'''
	'''for''' each state <math>j \in \{1,2,\ldots,K\}</math> '''do'''
