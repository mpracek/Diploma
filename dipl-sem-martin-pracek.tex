...
Gre torej za poseben primer prejšnjega primera, kjer je imamo sluèajni proces v diskretnih èasih, ki lahko v vsakem èasu pokaže le neko diskretno vrednost. 
...
\begin{primer}
	...
	Recimo, da $S_n$ pokaže $3$. Zanima nas verjetnost, da bo $S_{n+1} = 3$. Oèitno vidimo, da je ta verjetnost enaka $1/3$, saj velja $P(S_{n+1} = 3)  = P(X_{t+1})= 0)=P(X=0)=1/3$.Torej je vrednost naslednjega odvisna le od sedanjega stanja in tega, kaj bo pokazala $X_{t+1}$, ni pa pomembno, kako smo prišli do tega, da velja  $S_n = 3$, kar je toèno to, kar nam pove markovska lastnost. \\
\end{primer}
