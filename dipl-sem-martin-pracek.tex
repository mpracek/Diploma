...
\\
V nekaterih primerih pa velja še moènejša lastnost. 

\begin{trditev}{Moèna markovska lastnost}Naj velja, da imamo verjetnostni prostor s filtracijo $(\Omega,\mathbb{F},P,\mathbb{F}_{n})_{n\geq1})$. Naj bo $(X_n)_{n\geq1}$ zaporedje neodvisnih enako porazdeljenih sluèajnih spremenljivk ali vektorjev. Naj velja še, da je $T$ èas ustavljanja glede na naravno filtracijo, kjer velja $T<\infty$ skoraj gotovo. Potem je zaporedje $$(X_{T+1},X_{T+2},\ldots) (X_{1},X_{2},\ldots)$$ in je neodvisno od $\sigma$-algebre zgodovine èasa $\mathbb{F}_{T}$.
\end{trditev}
Iz te lahko izpeljemo tudi osnovno markovsko lastnost, èe predpostavimo da velja $t=T$, kjer je $T$ èas ustavljanja iz moène markovske lastnosti in $t$ èas iz \ref{markov}.
\begin{proof}
...
\begin{algoritem}[h]\label{alg:1}
	\nl \For{$i = 1, \ldots, k$} {
		\nl $\bm{u} = \bm{y}_j$ za poljuben j\;
		\nl \While{$\bm{t}$ se ne spreminja} {
			\nl $\bm{p} = \frac{\bm{X}^T\bm{u}}{\|\bm{X}^T\bm{u}\|}$\;
			\nl $\bm{t} = \bm{X}\bm{p}$\;
			\nl $\bm{q} = \frac{\bm{Y}^T\bm{t}}{\|\bm{Y}^T\bm{t}\|}$\;
			\nl $\bm{u} = \bm{Y}\bm{q}$\;}
		\nl \Return{p, u, q, t}\;
		\nl $\bm{X} = \bm{X} - \bm{tp}^T$\;
		\nl $\bm{X} = \bm{Y} - \bm{uq}^T$}
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\begin{itemize}
			...
			\item Zaporedje opazovanj $Y=(y_1,y_2,\ldots, y_T)$, tako da je $y_t==i$, èe velja da je opazovanje v èasu $t$ $o_i$
		...
		\For vsako stanje $j \in \{1,2,\ldots,K\}$ naj
		\State $T_1[j,1] \gets \pi_{}B_{jy_1}$
		\State $T_2[j,1] \gets 0$
		\EndFor
		\For vsako opazovanje $i = 2,3,\ldots,T$ naj
		\For vsako stanje $j \in \{1,2,\ldots,K\}$ naj
		\State $T_1[j,i] \gets \max_{k}{(T_1[k,i-1]\cdot A_{kj} \cdot B_{jy_i})} $
		\State $T_2[j,i] \gets \arg\max_{k}{(T_1[k,i-1]\cdot A_{kj}\cdot B_{jy_i})}$
		\EndFor
		\EndFor
		$z_T \gets \arg\max_{k}{(T_1[k,T])}$
		$x_T \gets s_z_T$
		\For $i \gets T,T-1,...,2$ naj
		\State $z_{i-1} \gets T_2[z_i,i]$
		\State$x_{i-1} \gets s_z_{i-1}$
		\EndFor
		\Return $X$
		
		\BState Output
		\State Najbolj verjetno zaporedje opazovanj $X=(x_1,x_2,\ldots,x_N)$
	\end{algorithmic}	
\end{algorithm}
\section{Uporaba}
...
Že v \ref{zahteva} sem omenil, da so finanèni donosi navadno "left-skewed", torej je mediana manjša od povpreèja. Prav zato naj bi bili skriti markovski modeli, \textbf{po prvem èlanku vira} bolj prirejeni za ugotavljanje padajoèih cen finanènih instrumentov. To bom preveril na veèih setih podatkov. Moj praktièni primer bom razdelil na veè razliènih delov. \\
