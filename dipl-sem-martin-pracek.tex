...
Zanima nas torej, kako bomo osnovali zaporedje $\lambda$, ki ga potrebujemo za maksimizacijo $P(O|\lambda)$. Izkaže se, da za maksimizacijo le tega potrebujemo še nekaj dodatnih formul, ki so rezultat Baum-Welchovega algortma.\\
...
Druga je $\Upsilon_{t}(i)$, ki nam pove verjetnost, da smo v èasu $t$ v stanju $i$.
$$\Upsilon_{t}(i) = \frac{\alpha_{t}(i)\beta_{t}(i)}{{P(O|\lambda)}$$
Vsota $\sum_{t=1}^{N}{\Upsilon_{t}(i)}$ nam pove prièakovano število prehodov iz stanja $i$.
\pagebreak
\section{Uporaba}
