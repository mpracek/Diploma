...
%Opiši primere ostalih treh markovskih modelov
\pagebreak
...
Opazovanja $x_t$ so razporejena po èasih, ko so bila zaznana, torej narašèajoèem èasu $t$. V realnosti pogosto velja, da so zaporedna opazovanja med seboj odvisna. To je predvsem oèitno, èe pomislimo na najbolj tipièen primer finanène èasovne vrste, in sicer vrednostnega procesa cene neke delnice. 
Glede na èase, v katerih gledamo rezultate naše èasovne vrste v grobem loèimo $3$ tipe:
\begin{enumerate}
	\item Èasovne vrste v zveznih èasih. Tu pridobivamo podatke za vsak trenutek znotraj nekega èasovnega intervala. Dober primer je tu EKG, torej elektrokardiogram.
	 
	 %Poišèi še kakšen primer za zvezen èas
	 
	 %Problem teh èasovnih vrst je, da jih dejansko ne moremo shraniti v celoti, saj bi morali v konèno velikost pomnilnika shraniti neskonèno mnogo vrednosti iz (naèeloma) realnih števil $\mathbb{R}$, ki imajo lahko tudi sama neskonèen decimalni zapis. Zato navadno uporabljamo približke teh vrst.
	\item Èasovne vrste z diskretnimi vrednostmi. Ideja v tem primeru je, da pridobivamo vrednosti, ki lahko zavzamejo le diskretne vrednosti, torej najveè števno neskonèno. Tipièen primer bi bil število klicev med $7.00$ in $8.00$ znotraj Ljubljane oziroma število nesreè znotraj enega dneva na avtocesti med Postojno in Ljubljano. 
	\item Zadnja vrsta je tista, s katero bom imel najveè opravka sam. Gre za diskretne èasovne vrste, torej vrste, ki vrednosti vzamejo le v doloèenih èasovnih trenutkih, npr. konec vsake ure, konec dneva, \ldots Tukaj lahko vrednosti zavzamejo kakršnokoli vrednost, pomembno je le, da te vrednosti vzamemo ob pravilnih trenutkih. Te vrste so uporabne tudi za aproksimiranje èasovnih vrst v zveznih èasih.s
\end{enumerate}
...
\definicija{Model èasovne vrste} za opazovane podatke ${x_t}$ je sluèajni proces $X_t$, kjer velja, da so $x_t$ realizacije v èasih $t$.\\
