...
torej velja
$$\rho_k = \frac{\sum_{i,j=1}^{m}E(Q_tQ_{t+k})|S_t =i,S_{t+k} =j)P(S_t =i)\Gamma^k_{ij}-\sum_{i=1}^{m}E(Q_t|S_t =i)P(S_t =i)\sum_{i=1}^{m}E(Q_{t+k}|S_t =i)P(S_{t+k} =i)}{\sum_{i=1}^{m}E(Q_t^2|S_t =i)P(S_t =i)-\sum_{i=1}^{m}E(Q_t|S_t =i)P(S_t =i)}$$
...
\includegraphics[scale=0.33]{slike/celota}
\includegraphics[scale=0.33]{slike/polovica}
%\begin{figure}%
%	\centering
