...
Preden lahko zaènemo govoriti o skritih markovskih modelih, moram nekaj povedati o osnovnih pojmih, ki jih bom uporabljal skozi celotno seminarsko nalogo.
\definicija{Markovska lastnost}
Naj bo ${\displaystyle (\Omega ,{\mathcal {F}},\mathbb {P} )}$ verjetnostni prostor s filtracijo $({\displaystyle ({\mathcal {F}}_{s},\ s\in I)})$ za neko urejeno množico ${\displaystyle I}$. Naj bo $({\displaystyle (S,{\mathcal {S}})})$ merljiv prostor. Stohastièni proces X, merjen na $({\displaystyle (S,{\mathcal {S}})})$ X =${\displaystyle X=\{X_{t}:\Omega \to S\}_{t\in I}}$ prilagojen na filtracijo ima markovsko lastnost, èe za vsak ${\displaystyle A\in {\mathcal {S}}}$ in vsak ${\displaystyle s,t\in I}$ kjer je ${\displaystyle s<t}$, velja da je,
...
Poznamo veè razliènih vrst Markovih modelov. Najbolj znana je Markovska veriga. 


\definicija{Markovska veriga} je zaporedje sluèajnih spremenljivk, ki zadošèa markovski lastnosti, kjer lahko vsa stanja opazujemo. 

\begin{table}[]
	\begin{tabular}{lcc}
		& Sistem lahko v celoti opazujemo & Sistem lahko opazujemo le delno             \\
		\multicolumn{1}{c}{Avtonomen sistem} & Markovska veriga                & Skriti markovski model                      \\
		Kontorliran sistem                   & Markovski proces odloèanja      & Delno opazovalen maproces odloèanja
	\end{tabular}
\end{table}s
