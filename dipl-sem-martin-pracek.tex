...
\begin{geslo}{Gaussova mešanica}{Gaussova mešanica je porazdelitvena funkcija z gostoto, ki jo lahko zapišemo kot tehtano povpreèje normalnih gostot; $$f(x) = \sum_{j = 1}^{M}{c_{j}} N(x;\mu_{j}, \sigma_{j}^2)$$. Veljati mora še, da je $$\sum_{j = 1}^{M}{c_{j}} = 1$$ in da je $N(x;\mu_{j}, \sigma_{j}^2)$ gostota normalne porazdelitvene funkcije.} 
\end{geslo}\\
\begin{geslo}{Akaikejev informacijski kriterij}{Akaikejev informacijski kriterij oziroma AIC je merilo relativne kakovosti modela glede na preostale modele za iste podatke.Naj velja, da imamo podatke s $k$ ocenjenimi parametri in $L$ maksimalna vrednost funkcije verjetja. AIC izraèunamo kot $$AIC = 2k - 2ln(L)$$ AIC primerja veè razliènih modelov po kakovosti, kjer je najboljši tisti z najmanjšo vrednostjo.}
\end{geslo}\\
\begin{geslo}{Baum-Welchov algoritem}{Je algoritem, s katerim poišèemo neznane parametre skritih markovskih modelov.}
\end{geslo}\\%malo razlage
\begin{geslo}{Dinamièna bayesianska mreža}{Je bayesianska mreža, v kateri so povezane spremenljivke, ki ležijo v dveh sosednjih èasovnih korakih.}
\end{geslo}\\%Dodaj malo razlage
\begin{geslo}{Pogojna vrednost ob padcu}{Pogojna vrednost ob padcu CVaR je nam pove verjetnost, da se bo donos portfelja od prièakovane vrednosti pomaknil za veè kot tri standarde odklone.}
\end{geslo}\\%Expected shortfall
...
To definicijo lahko v diskretnem primeru še razširimo, èe velja še, da je $I = \mathbb{N}$. Potem velja $$\mathbb{P}(X_n=x_n\mid X_{n-1}=x_{n-1}, \dots, X_0=x_0)=\mathbb{P}(X_n=x_n\mid X_{n-1}=x_{n-1})$$
\\

%Tukaj dopiši še kar nekaj o teh modelih, da se tabela prestavi na naslednjo stran
%Ugotovi napako tabele
\begin{table}[h!]
	\begin{tabular}{lllll}
		\cline{1-3}
	...
	\end{tabular}
\end{table}
%Uporabnost markovske lastnosti

Zaradi svoje široke aplikativnosti se markovski modeli uporabljajo v razliènih vrstah predikcije. V grobem lahko te vrste delimo na:
\begin{itemize}
...
\begin{primer}{Primer žabe v ribniku}
%Dodaj kratice
...
%Dodaj še kaj o baynesianskih mrežah, primer, razlaga.
...
\\
Ideja za delovanje skritih markovskih modelov izhaja iz dela Andrewa Viterbija na podroèju komunikacije. V komunikacijskih storitvah šumi namreè predstavljajo veliko težavo, zato se je ameriško-italijanski elektrièni inžiner domislil algoritma, s katerimi bi iz seta podatkov izloèili šume. To si lahko predstavljamo kot da bi si na primer želeli na fiksnem mestu v elektriènem tokokrogu doloèiti naboj $Q(t)$ glede na naboj ob èasu $s$, kjer $s<t$. Zaradi napake pri merjenju $Q(s)$ ne moremo toèno doloèiti, saj zaznamo neko zmoteno razlièico $\widehat{Q(s)}$. Želimo si torej naèin, ki bi èim bolj uèinkovito izloèil te napake. \\
%Podaljšaj opis, dodatne razlage. Poišèi še kaj po virih. Dopiši!!
%Možnosti za eno in veè spremenljivk, problemi modela veè spremenljivk
%Bolj podroben opis
...
\begin{itemize}
	...
	\item matriko $C$, ki predstavlja koeficiente $c_{ij}$, ki so faktorji v Gaussovi mešanici. To matriko imenujemo tudi Izhodno matriko.
...
\subsection{Natanèna doloèitev zaèetnih vrednosti}\label{zaèetno}
...
\Return $X$
%Popravi napako v algoritmu 
\end{algorithm}
%Dodaj algoritem za delovanje celotnega HMM
...
Uporaba skritih markovsih modelov spada pod 
...
\begin{primer}{Problem izbire portfelja}
	...
	Predpostavimo, da imamo v trenutnem èasu kapital $M$, ki ga lahko vložimo v $N$ razliènih vrednostnih papirjev. Odloèiti se moramo, kako bomo naš kapital razdelili, da bomo maksimizirali svoj donos in hkrati minimizirali tveganje.\\
	Vsak vrednostni papir ima donos $R_j$, kjer velja $j \in{1,\ldots,N}$. Za vsak $j$ je $R_j$ sluèajna spremenljivka, katere porazdelitev aproksimiramo z diskretno sluèajno spremenljivko. Doloèimo še vektor $x = (x_1,\ldots,x_N)$, kjer nam $x_j$ pove, kakšen delež kapitala smo vložili v posamezen vrednostni papir, to je $$x_j = \frac{M_j}{M}$$ èe velja $\sum_{j=1}^{N}{M_j}=M$. Ob koncu investicije bomo torej zaslužili $$R_x = \sum_{j=1}^{N}{x_jR_j}$$\\
	To nalogo lahko predstavimo kot linearni program s pomoèjo pogojne vrednosti ob padcu CVaR. CVar je mera tveganja, ki nam pove 
\end{primer}
...
Že v \ref{zahteva} sem omenil, da so finanèni donosi navadno "left-skewed", torej je mediana manjša od povpreèja. Prav zato naj bi bili skriti markovski modeli, po \cite{prvivir} bolj prirejeni za ugotavljanje padajoèih cen finanènih instrumentov. To bom preveril na veèih setih podatkov. Moj praktièni primer bom razdelil na veè razliènih delov. \\
...
%standardni odklon, varianca, maksimalni in minimalni odklon, grafièni prikaz na istem grafu. 
...
\bibitem{prvivir}
D.~Roman,G.~Mitra,N.~Spagnolo \emph{Hidden Markov models for financial optimization problems}, IMA Journal of Management Mathematics \textbf{21} (2010) 111--129.
